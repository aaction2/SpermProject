...
public class MyPreferenceFragment extends PreferenceFragment implements OnSharedPreferenceChangeListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		final String cameFrom = bundle.getString("cameFrom");

		// find the other bundles only if you come from the MainActivity.
		if (cameFrom != null && cameFrom.equals("MainActivity")) {
			if (MyDebug.LOG)
				Log.d(TAG, "came from the " + cameFrom);
...
//        }
        ...
        /*{
			...
			// odd but one of the choices is still on.
			// just delete it manually, ain't doing no debugging.
			PreferenceCategory prefCat = (PreferenceCategory) findPreference("preference_category_online");
			CheckBoxPreference checkBoxPref = (CheckBoxPreference) findPreference("preference_use_camera2");
			prefCat.removePreference(checkBoxPref);
			if (MyDebug.LOG)
				Log.d(TAG, "Removed preference_use_camera2");
		}
            ...
            pref.setOnPreferenceClickListener(new OnPreferenceClickListener() {
                ...
                public boolean onPreferenceClick(Preference arg0) {
                	if( pref.getKey().equals("preference_about") ) {
		                ...
		                final String about_string = "Sperm3000.\n" +
				                "Version Code: 1.0\n" +
				                "National Technical University of Athens\n" +
				                "Depamrtment of Mechanical Engineering\n" +
				                "Systems Biology and Bioengineering Lab\n" +
				                "\n" +
				                "The application was created as part of a six-month project by the following people:\n\n" +
				                "Nikos Koukis - nickkouk@gmail.com\n" +
				                "Konstantinos Derventzis - kostas_der@hotmail.com\n" +
				                "Athina Parousi-Papadopoulou - athinadasx@hotmail.com\n" +
				                "Angelos Mavroudis - angelosmu@gmail.com";
	...
	// nickkouk
	public void removeUnwantedPrefs() {
		if (MyDebug.LOG)
			Log.d(TAG, "RemoveUnwantedPrefs");

		// get a hold of the PreferenceScreen
		// http://stackoverflow.com/a/31361286/2843583
		PreferenceScreen preferenceScreen = (PreferenceScreen) findPreference("preferenceScreen");
		// remove all the main categories from the preference screen of the user
		PreferenceCategory prefCat = (PreferenceCategory) findPreference("preference_category_camera_controls");
		preferenceScreen.removePreference(prefCat);
		prefCat = (PreferenceCategory) findPreference("preference_category_camera_effects");
		preferenceScreen.removePreference(prefCat);
		prefCat = (PreferenceCategory) findPreference("preference_category_camera_quality");
		preferenceScreen.removePreference(prefCat);
	}
